...
class Grabber:
    def __init__(self,cls_image,monitor):
        ...
        bmp = ctypes.windll.gdi32.CreateCompatibleBitmap(srcdc, monitor['width'], monitor['height'])
    ...
    def grab(self):
        try:
            ...
            bits = ctypes.windll.gdi32.GetDIBits(
                self.memdc, self.bmp, 0, self.monitor['height'], data, self.bmi, DIB_RGB_COLORS)
            if bits != self.monitor['height']:
                del data
    ...
    def __del__(self):
        # Clean up
        if self.srcdc:
            ctypes.windll.gdi32.DeleteObject(self.srcdc)
        if self.memdc:
            ctypes.windll.gdi32.DeleteObject(self.memdc)
        if self.bmp:
...
class MSS(MSSBase):
    ...
    def grabber(self, monitor):
        ...
        return Grabber(monitor,self.cls_image)
    ...
    def grab(self, monitor):
        ...
        """
        with self.grabber(monitor) as g:
            return g.grab()
